...
const pendingProfile = new Map(); // lineUid -> { step, data } 設定資料問卷進度
...
const HELP_TEXT =
  '📖 練食記 使用說明\n' +
  '\n' +
  '・設定資料：打「設定資料」算你的專屬熱量＋蛋白，課表也依目標調整\n' +
...
const RESERVED_EXACT = new Set([
  ...
  '連動', 'Apple連動', '運動連動',
  '設定資料', '個人化', '個人資料', '我的資料', '重新設定資料', '設定個人資料',
  '喝水', '水', '飲水', '喝水記錄',
  '提醒', '提醒設定', '記帳提醒', '定時提醒',
...
// 使用者目標（設過資料才有）：增肌/減脂/維持；沒設回 null，課表就用預設組×次
function userGoal(user) {
  const g = user && user.goal_mode;
  return g === '增肌' || g === '減脂' || g === '維持' ? g : null;
}

// 是否已完成設定資料
function hasProfile(user) {
  return !!(user && user.pweight && user.height && userGoal(user));
}

// 從資料庫的 user 列還原出 profile 物件（給重看計畫用）
function profileFromUser(user) {
  return {
    ...
    height: user.height,
    weight: user.pweight,
    bodyfat: user.bodyfat != null ? user.bodyfat : null,
...
// 開始設定資料問卷
function startProfile(lineUid) {
  pendingProfile.set(lineUid, { step: 'sex', data: {} });
  return [
    text(
      '來設定你的專屬資料 📋\n算出你的每日建議熱量、蛋白質，課表也會依目標調整。\n（隨時打「取消」中止）\n\n1/7　生理性別？',
      sexQuickReply
    ),
  ];
}
  ...
  switch (state.step) {
    case 'sex': {
      if (content !== '男' && content !== '女') {
        return [text('請點「男」或「女」', sexQuickReply)];
      }
      data.sex = content;
      state.step = 'age';
      return [text('2/7　年齡幾歲？（打數字）', cancelQuickReply)];
    }
    case 'age': {
      const n = Number(content);
      if (!Number.isInteger(n) || n < 10 || n > 100) {
        return [text('年齡請打 10-100 的數字', cancelQuickReply)];
      }
      data.age = n;
      state.step = 'height';
      return [text('3/7　身高幾公分？（例：171）', cancelQuickReply)];
    }
    case 'height': {
      const n = Number(content);
      if (!(n >= 120 && n <= 230)) {
        return [text('身高請打 120-230 的數字（公分）', cancelQuickReply)];
      }
      data.height = n;
      state.step = 'weight';
      return [text('4/7　體重幾公斤？（例：73.8）', cancelQuickReply)];
    }
    case 'weight': {
      const n = Number(content);
      if (!(n >= 30 && n <= 250)) {
        return [text('體重請打 30-250 的數字（公斤）', cancelQuickReply)];
      }
      data.weight = n;
      state.step = 'bodyfat';
      return [text('5/7　體脂率幾 %？沒量過可跳過（有填會更準）', bodyfatQuickReply)];
    }
    case 'bodyfat': {
      if (['跳過', '略過', '沒有', '沒量過'].includes(content)) {
        data.bodyfat = null;
      } else {
        const n = Number(content.replace('%', ''));
        if (!(n >= 3 && n <= 60)) {
          return [text('體脂請打 3-60 的數字，或按「跳過」', bodyfatQuickReply)];
        }
        data.bodyfat = n;
      }
      state.step = 'activity';
      return [text('6/7　平常活動量？（含健身＋體能老師工作，多半是中度或高度）', activityQuickReply)];
    }
    case 'activity': {
      if (!['久坐', '輕度', '中度', '高度'].includes(content)) {
        return [text('請點四個活動量之一', activityQuickReply)];
      }
    ...
    case 'goal': {
      if (!['增肌', '減脂', '維持'].includes(content)) {
        return [text('請點「增肌／減脂／維持」', goalQuickReply)];
      }
      data.goal = content;
    ...
    default:
      pendingProfile.delete(lineUid);
      return [text('設定流程有點狀況，請再打一次「設定資料」')];
  }
}
...
async function handleEvent(event) {
  ...
  // ---- 設定資料（個人化熱量＋課表）----
  if (content === '設定資料' || content === '個人化' || content === '設定個人資料' || content === '重新設定資料') {
    clearPending(lineUid);
    return startProfile(lineUid);
  }
  // 「我的資料／個人資料」：設過就看計畫，沒設過就開始設定
  if (content === '我的資料' || content === '個人資料') {
    if (hasProfile(user)) {
      const profile = profileFromUser(user);
      return [buildPlanFlex(profile, computePlan(profile))];
    }
    clearPending(lineUid);
    return startProfile(lineUid);
  }
  // 問卷進行中：把這句當成答案；若改打其他指令就放掉問卷、往下照常處理
  if (pendingProfile.has(lineUid)) {
    if (isCommandLike(content)) {
      pendingProfile.delete(lineUid);
    } else {
  ...
  if (intent.type === 'overview') {
    ...
    const card = buildOverviewFlex(summary, {
      calTarget,
      waterGoal,
      proteinTarget: user.protein_target || null,
